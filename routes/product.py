...
def salvar_produto():
    ...
    required_fields = ["nome", "codigo_referencia", "marca", "id_api_externa"]
    for field in required_fields:
        if not dados.get(field):
            return (
                jsonify(
                    {"success": False, "error": f"O campo '{field}' é obrigatório."}
                ),
                400,
            )

    # Tratamento de valores numéricos de forma segura
    try:
        ...
        return (
            jsonify(
                {"success": False, "error": f"Erro ao processar dados de preço: {e}"}
            ),
            400,
        )
...
def get_produtos_carrinho():
    try:
        ...
        # Otimizado para usar o método to_dict se você o definir na classe
